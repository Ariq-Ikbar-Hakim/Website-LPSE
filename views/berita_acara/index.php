    ...
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-fade-in-up">
        ...
        <form action="index.php?page=ba_manual_upload" method="POST" enctype="multipart/form-data" class="p-6 overflow-y-auto max-h-[80vh]">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="bg-amber-50 p-4 rounded-xl border border-amber-200">
                        <label class="block text-sm font-bold text-amber-900 mb-2">Unggah Tanda Tangan PPK (Opsional)</label>
                        <input type="file" name="signature_image_ppk" accept="image/png, image/jpeg" class="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-amber-100 file:text-amber-700 hover:file:bg-amber-200 cursor-pointer">
                        <p class="text-xs text-amber-700 mt-2">Format PNG/JPG. Jika ada, akan jadi QR Code PPK.</p>
                    </div>
                </div>
            </div>
            <div class="mt-6 pt-4 border-t border-slate-100 flex justify-end gap-2">
                ...
                <button type="submit" onclick="return confirm('Yakin ingin mengunggah Berita Acara manual ini? Paket akan otomatis selesai.')" class="px-5 py-2.5 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-md transition">Unggah & Selesaikan</button>
            </div>
        </form>
    </div>
</div>

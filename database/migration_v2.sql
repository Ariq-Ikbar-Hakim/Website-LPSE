-- ============================================================
-- LPSE APELBAJA — DATABASE MIGRATION v2
-- Database: db_apelbaja
-- Dibuat: 2026-06-15
-- ============================================================
-- BACKUP DULU SEBELUM MENJALANKAN SCRIPT INI!
-- mysqldump -u root apelbaja_ppk > backup_apelbaja_ppk_before_v2.sql
-- ============================================================
...
CREATE TABLE IF NOT EXISTS assignment_transfer (
    ...
    created_at      TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    INDEX idx_at_paket_id (paket_id),
    CONSTRAINT fk_at_paket_id       FOREIGN KEY (paket_id)       REFERENCES paket(id) ON DELETE CASCADE,
...
-- ============================================================
-- 11. TABEL: log_paket (dipertahankan dari v1, ditambah kolom)
-- ============================================================
...
-- ============================================================
-- CATATAN MIGRASI DARI db lama (apelbaja_ppk):
-- Jika ada data existing, jalankan query berikut SETELAH
-- membuat database baru:
--
-- INSERT INTO db_apelbaja.users
--   (nip, nama, email, password, no_telp, opd, sub_unit_opd,
--    jabatan_aktif, sk_nomor, sk_mulai, sk_sampai, sk_file,
--    keterangan, status_aktif, created_at)
-- SELECT
--   nip, nama, email, password, no_telp, opd, sub_unit_opd,
--   hak_akses, sk_nomor, sk_mulai, sk_sampai, sk_file,
--   keterangan, status_aktif, created_at
-- FROM apelbaja_ppk.users;
--
-- Kemudian migrasi tabel paket dengan menambahkan pp_id default:
-- (Perlu pilih PP yang akan di-assign secara manual setelah migrasi)
-- ============================================================

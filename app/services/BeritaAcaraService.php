...
class BeritaAcaraService
{
    ...
    public function sign(int $paketId, string $signatureImagePath = ''): bool
    {
        ...
        if ($newStatus === 'tanda_tangan_kedua') {
            ...
            if (isset($ba['jenis_ba']) && $ba['jenis_ba'] === 'manual') {
                $bulanIndo = ['','Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'];
                $tanggalPdf = date('d', strtotime($ba['tanggal_ba'])) . ' ' . $bulanIndo[(int)date('n', strtotime($ba['tanggal_ba']))] . ' ' . date('Y', strtotime($ba['tanggal_ba']));
                ...
                $html .= '<p class="subtitle">Nomor: ' . htmlspecialchars($ba['nomor_ba']) . '</p>';
                ...
                $html .= '<table class="data"><tbody>
                            <tr><td>Nama Paket</td><td>' . htmlspecialchars($paketInfo['nama_paket'] ?? $ba['konten']) . '</td></tr>
                ...
                $html .= '<br><p class="sig-name">' . htmlspecialchars($pp['nama'] ?? '') . '</p><p class="sig-nip">NIP: ' . htmlspecialchars($pp['nip'] ?? '') . '</p></td>';
                $html .= '<td><b>Pejabat Pembuat Komitmen (PPK)</b><br>';
                if ($ppk && file_exists(BASEPATH . '/' . $ppk['qr_image_path'])) {
                    $imgDataPpk = base64_encode(file_get_contents(BASEPATH . '/' . $ppk['qr_image_path']));
                    $html .= '<img src="data:image/png;base64,' . $imgDataPpk . '" class="qr-img">';
                }
                $html .= '<br><p class="sig-name">' . htmlspecialchars($ppk['nama'] ?? '') . '</p><p class="sig-nip">NIP: ' . htmlspecialchars($ppk['nip'] ?? '') . '</p></td>';
    ...
    public function generateManualInApp(int $paketId, string $nomorBa, string $tanggalBa, string $kontenBa, string $signatureImagePath, string $signatureImagePathPpk = ''): array
    {
        ...
        if (!$ba) {
            $data = [
                ...
                'status' => 'ditandatangani_pp',
                'jenis_ba' => 'manual'
            ];
            $baId = $this->baModel->create($data);
            if (!$baId) {
                throw new \Exception("Gagal membuat Berita Acara. Pastikan Nomor BA belum digunakan oleh paket lain.");
            }
            $ba = $this->baModel->findById($baId);
        } else {
            // Update BA yang sudah ada
            $stmt = $conn->prepare("UPDATE berita_acara SET nomor_ba = ?, tanggal_ba = ?, konten = ?, hash_konten = ?, status = 'ditandatangani_pp', jenis_ba = 'manual' WHERE id = ?");
        ...
        // Update Paket Status
        $paketService = new PaketService($conn);
        $paketService->updateStatus($paketId, 'disetujui', 'Berita Acara Manual di-generate dan ditandatangani oleh PP (menunggu PPK)');
